	...
	while (bufPos < endPos) {
		...
		case 0x61: /* user data */
			...
			if (buffer[bufPos++] != 0x02) {
				if (DEBUG) printf("wrong type for presentation selector\n");
				return -1;
			}
			...
			if (buffer[bufPos++] != 0xa0) {
				if (DEBUG) printf("wrong type for user data\n");
				return -1;
			}
			...
			if (bufPos < 0) {
				if (DEBUG) printf("PRES: wrong parameter length for user data\n");
				return -1;
			}

			self->nextPayload.buffer = buffer + bufPos;
			self->nextPayload.size = len;
